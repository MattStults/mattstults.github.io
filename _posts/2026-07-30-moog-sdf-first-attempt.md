...
1. **Run the cells that never ran** — R2 (the register axis, the only corpus factor never tested), R4, R5, and R6 on its trigger. Registered details for that run. The on-box prune is retired for explicitly planned cells: it is what dropped R2 the first time, and its premise — paraphrase as the lever — is falsified above. Full-FT cells write no intermediate checkpoints (R4's crash was a ~17GB optimizer checkpoint on a 24GB disk). The LoRA cells train at the revised recipe (lr 3.5e-5, a third of R1's), so R2's register comparison runs against the docs-only replicate at the same learning rate — arm B — not against R1, and it ships with the measured per-lane exposure delta (about 3% fewer rule-string occurrences under transcripts). The decide legs of the existing adapters are re-read under the fixed prompt with a local re-read tool; R1 saved no completions, so its decide legs are unrecoverable; arm A, its byte-identical replicate, stands in for that recipe.
2. **If no cell clears the screen and the forgetting bound, take the registered exit**: knowledge SFT, with the decomposition's own verdicts as the documented reason — for every cell so far that verdict is trainer damage, not a register failure, and the exit is taken on that stated basis, not as "the register wasn't reached." The knowledge is still in the weights and the follow-on program survives; the acquisition-realism claim scopes down one rung.
